...
add_action('wp_logout','logout_check');
...
function logout_check(){
	...
	$sqlins = "UPDATE wp_login_check set loggedin = $val where username = '$current_username'";
	$retvalins = mysql_query( $sqlins, $conn );

	if(!$retvalins){
		die($current_user_name);
	}else{
...
{
	global $bp;
	$bp->bp_options_nav[$bp->groups->current_group->slug]['calendar'] = false;
