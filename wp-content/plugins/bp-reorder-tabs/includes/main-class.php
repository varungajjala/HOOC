...
class BuddyPress_Reorder_Tabs_Plugin {
					...
					foreach( $subnavs as $subnav=>$subnav_props ){
						...
						if( isset( $bp->bp_options_nav[$topnav] ) ){
							foreach( $bp->bp_options_nav[$topnav] as $index=>$opt_nav ){
								if( isset( $opt_nav['slug'] ) && $opt_nav['slug']==$subnav ){
	...
	public function save_profile_navs_info(){
		...
		// Loop through each navigation item
		foreach( (array) $bp->bp_nav as $nav_outer ){
			$item_outer = array(
				'name'		=> $nav_outer['name'],
			...
			if( isset( $bp->bp_options_nav[$nav_outer['slug']] ) ){
				foreach( $bp->bp_options_nav[$nav_outer['slug']] as $nav_inner ){
					$item_inner = array(
						'name'		=> $nav_inner['name'],
