...
function wp_authenticate_username_password($user, $username, $password) {
	...
	$sql = "SELECT username from wp_login_check where username = '$username'";
	$retval = mysql_query($sql, $conn);
	$row = mysql_fetch_assoc($retval);
	
	if($row['username'] == ""){
		$sqlins = "INSERT INTO wp_login_check VALUES ('$username', $val)";
		$retvalins = mysql_query( $sqlins, $conn );
	}else{
		$sqlupd = "UPDATE wp_login_check set loggedin = 1 where username='$username'";
		$retvalupd = mysql_query($sqlupd, $conn);
	}
